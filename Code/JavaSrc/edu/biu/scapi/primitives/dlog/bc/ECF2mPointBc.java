...
public class ECF2mPointBc extends ECPointBc{
	 ...
	 */
	public ECF2mPointBc(BigInteger x, BigInteger y, DlogGroup curve){
	 ...
	 * @param curve
	 */
	public ECF2mPointBc(DlogGroup curve){
		//the DlogGroup that matches this class is BcAdapterDlogEC
		if (curve instanceof BcAdapterDlogEC){
			...
			for(int i=0; i<2*m; i++){
				...
				ECFieldElement.F2m xElement = new ECFieldElement.F2m(m, k1, k2, k3, x);
				ECFieldElement.F2m aElement = new ECFieldElement.F2m(m, k1, k2, k3, desc.getA());
				ECFieldElement.F2m bElement = new ECFieldElement.F2m(m, k1, k2, k3, desc.getB());
				//compute x^3
				...
				ECFieldElement.F2m fx4 = (F2m) fx.multiply(new ECFieldElement.F2m(m, k1, k2, k3, new BigInteger("4")));
				//compute x^2-4f(x)
				ECFieldElement.F2m delta = (F2m) xElement.square().add(fx4.negate());
				ECFieldElement.F2m yVal = null;
				ECFieldElement.F2m two = new ECFieldElement.F2m(m, k1, k2, k3, new BigInteger("2"));
	...
	boolean checkValidity(BigInteger x, BigInteger y, ECGroupParams params) {
		...
		if (params instanceof ECF2mGroupParams){
			ECF2mGroupParams desc = (ECF2mGroupParams) params;
			int m = desc.getM(); //get the field size
			...
			ECFieldElement.F2m xElement = new ECFieldElement.F2m(m, k1, k2, k3, x);
			ECFieldElement.F2m yElement = new ECFieldElement.F2m(m, k1, k2, k3, y);
			ECFieldElement.F2m aElement = new ECFieldElement.F2m(m, k1, k2, k3, desc.getA());
			ECFieldElement.F2m bElement = new ECFieldElement.F2m(m, k1, k2, k3, desc.getB());
			 ...
			 */
			//compute x^3
			System.out.println("x^2 is:"+xElement.square().toBigInteger());
			ECFieldElement.F2m x3 = (F2m) xElement.square().multiply(xElement);
			//compute ax^2
			ECFieldElement.F2m ax2 = (F2m) aElement.multiply(xElement.square());
			//compute x^3+ax^2+b
			ECFieldElement.F2m result = (F2m) x3.add(ax2).add(bElement);
			System.out.println("result r is:"+result.toBigInteger());
			//compute xy
			ECFieldElement.F2m xy = (F2m) yElement.multiply(xElement);
			//compute y^2+xy
			ECFieldElement.F2m y2 = (F2m) yElement.square().add(xy);
			System.out.println("result l is:"+y2.toBigInteger());
